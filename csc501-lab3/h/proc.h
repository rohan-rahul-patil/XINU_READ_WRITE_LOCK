...
struct	pentry	{
	...
	int	pwaitret;
	int 	pinh;  			/* current inhereted priority of the process */
	int	locks_held[NLOCK];       /* all the locks held by the process */
	int	lock_id;		/* lock descriptor in whose waitist the process is blocked */
	int	wait_time;		/* time of a process waiting on a certain lock */
	int	lock_type;		/* type of lock the process is holding READ, WRITE, NONE */
};
